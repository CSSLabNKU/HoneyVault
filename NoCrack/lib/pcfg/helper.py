...
BASE_DIR = os.getcwd()
# 根据需要更改
from lib.pcfg.honeyvault_config import MAX_INT, DEBUG, PRODUCTION  # MEMLIMMIT
...
class random:
    ...
    def randints(s, e, n=1):
        ...
        if not DEBUG:
            arr = [
                s + a % (e - s) for a in struct.unpack("<%dL" % n, os.urandom(4 * n))
            ]
        else:
            orig_random.seed(0)
    ...
    def choice(arr):
        ...
        assert i < len(
            arr
        ), "Length exceeded by somehow! Should be < {}, but it is {}".format(
            len(arr), i
        )
        return arr[i]
...
def bin_search(A, p, s, e):
    """Search p in A"""
    mid = (s + e) / 2
    if (mid == 0 or A[mid - 1][1] <= p) and A[mid][1] > p:
        return A[mid][0]
    elif A[mid][1] <= p:
...
def convert2group(t, totalC, n=1):
    if n == 1:
        ...
        return [
            t + c * totalC for c in random.randints(0, (MAX_INT - t) // totalC, n=n)
        ]
...
def getIndex(p, A):
    """在列表 A 中找到一个索引，使得从该索引开始的元素之和大于或等于给定的值 p。"""
    p %= A[-1]
    i = 0
...
def save_pickle(item, file):
    fpkl = open(file, "wb")
    pkl.dump(item, fpkl, pkl.HIGHEST_PROTOCOL)
    fpkl.close()
