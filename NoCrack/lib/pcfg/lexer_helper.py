...
class RuleSet(object):
    ...
    def __init__(self, l=None, r=None, d=None):
        self.G = defaultdict(OrderedDict)#空的
        if l and r:
            self.add_rule(l, r)
        if d:
            for k, v in list(d.items()):
                #print(k,v)
                self.G[k].update(v)
        #print('另一个角落的',self.G) #[('!!', 782), ('!!!', 119)] 长这样，很奇怪

    def add_rule(self, l, r, f=0, rule=None):
        if rule:
            l, r = rule
        self.G[l][r] = self.G[l].get(r, 1) + f
        #如果R已经有这个（r,l）了,比如new_sub_tg里面的野生R，初始化的时候野生R（因此自己的R也会有）里面已经有('G','W1,G'),这里装模做样再操作一遍就好了，什么都没改变，
        #如果是新的结构，比如('G','W8'),赋频数为1。后面 sub_tg.finalize()的时候，sub_tg.R会利用fix()函数继续更新这个频数，然后再进一步得到sub_tg.G。
...
$""".format(**{'mm': mm, 'yy': yy, 'yyyy': yyyy,
    ...
    def IsDate(self, s):
        # 源代码是match，我们应该修改成fullmatch，确保整个字符串严格符合日期格式
        m = re.fullmatch(self.date_regex, s) 
...
if __name__ == "__main__":
    ...
    Date(T_rules=['Y']).IsDate('1995sjgjh')
